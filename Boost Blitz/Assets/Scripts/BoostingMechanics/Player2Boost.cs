...
public class Player2Boost : MonoBehaviour
{
    ...
    void CheckBoost()
    {
        if (Input.GetKeyDown(KeyCode.J) && !isBoosting && cooldownTimer <= 0.0f)
        {
            isBoosting = true;
    ...
    void OnGUI()
    {
        if (cooldownTimer > 0.0f)
        {
            GUI.Box(new Rect(10, 550, 500, 70), "Boost Cooldown: " + cooldownTimer.ToString("F2"), BoostCooldownStyle);
        }
        else if (isBoosting)
        {
            GUI.Box(new Rect(10, 550, 400, 70), "Boost Time: " + boostTimer.ToString("F2"), WhileBoostingStyle);
        }
        else
        {
            GUI.Box(new Rect(10, 550, 400, 70), "Boost Ready!", BoostReadyStyle);
        }
    }
}